...
def w_from_drift(z, yd, year, Lfloat, dt=30, dftn=None):
    """
...
def _prep_ocsp_drift(GDrive_root, year):
    site_root = GDrive_root + 'Data/' + 'OCSP/'
    ...
    with xr.open_dataset(flux_dir+'OCSP_fluxes_'+str(year)+'.nc') as F:
        ustar = F.ustar.sel(time=bld.time)
        wbnet = F.wbnet.sel(time=bld.time)
        wb0 = F.wb0.sel(time=bld.time)
        Bnsw = -(wbnet-wb0)
        Bf, wbf = sot.get_BFs_Jwt(wb0, Bnsw, bld, Jwtype='II')
        theta_tau = np.arctan2(F.tauy, F.taux) # not filtered
    
    wstar = np.cbrt(wbf*bld)
    ...
    for i in tqdm(Idrift, desc='Compute w for each drift'):
        ...
        df, spec = w_from_drift(Zc[idf], ydP[idf], year, Lfloat, dt=1, dftn=dftn)
    ...
    dftb['BF'] = wb0.sel(time=dftb.time)*(1+dftb.zoh) + \
                 sot.get_wbr_Jwt(Bnsw.sel(time=dftb.time), dftb.mz, dftb.bld, Jwtype='II')
...
_prep_ocsp_drift(Groot, 2011)
_prep_ocsp_drift(Groot, 2012)