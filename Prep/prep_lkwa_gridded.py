...
def _prep_lkwa_gridded(GDrive_root, floatID):
    ...
    non_empty = ~(np.isnan(bnd2d).sum(axis=0)>=(bnd2d.shape[0]-3)).any(axis=-1) # at least 3 data points in one profile
    bnd2d = bnd2d[:,non_empty,:]
    ...
    gridded = xr.Dataset(
        ...
        attrs={'description': 'Profile and gridded data from the Lagrangian float '+str(floatID)+' deployed in Lake Washington (2011)',
               'note': '2D gridded fields are from Gaussian interpolation; mixed layer depths are linearly interpolated.'})

    nc_name = 'LKWA_gridded_'+str(floatID)+'.nc'
...
_prep_lkwa_gridded(Groot, 71)
_prep_lkwa_gridded(Groot, 72)