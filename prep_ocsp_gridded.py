...
from constants import g, cp, rho0, nan
from sys import platform
from scipy import stats


def _prep_ocsp_gridded(GDrive_root, year):
    ...
    # only fill top for those contributed to the previous binning
    ipf_notop = (Zpf[0,:] < mZtop) & (Zpf[0,:] >= -5)
    ig = ~np.isnan(bnd_top[:,0])
    nt = ydprof.size
    CTpf = np.r_[np.full((1,nt), nan), CTpf]
    ...
    MZ = np.stack([CTpf, SApf, Zpf], axis=-1)
    igood = ~np.isnan(Zpf)
    ...
    gridded = xr.Dataset(
        ...
        attrs={'description': 'Profile and gridded data from the Lagrangian float deployed in '+str(year)+' near ocean climate station Papa',
               'note': '2D gridded fields are from Gaussian interpolation; mixed layer depths are linearly interpolated.'})

    nc_name = 'OCSP_gridded_'+str(year)+'.nc'
...
_prep_ocsp_gridded(Groot, 2011)
_prep_ocsp_gridded(Groot, 2012)